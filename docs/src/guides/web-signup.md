...
2. **Google OAuth** — in a Google Cloud project, configure the OAuth
   ...
   screen when you're ready for users outside your test list.

## Configuration

Signup is enabled only when **all five** of these are set (setting only
some is a startup error):

| Variable | Meaning |
| --- | --- |
| `OYSTER_PUBLIC_BASE_URL` | Public base URL, e.g. `https://oyster.example.com`; used to build the OAuth redirect URI |
...
Admin keys issued via the web are indistinguishable from CLI-issued
...
CLI (direct database access, same as `oysterd app`):
...
Cloudflare publishes dummy Turnstile keys that need no account and skip
real challenges:
...
server (`scripts/signup-testbed.sh`); for manually exercising real
...
- Web key issuance/revocation is recorded in `audit_events`
  (`admin_key.issued_via_web` / `admin_key.revoked_via_web`).
