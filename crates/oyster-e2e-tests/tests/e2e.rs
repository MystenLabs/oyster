...
async fn e2e_blob_lifecycle() {
    // Install the default CryptoProvider (same as oyster's main.rs).
    ...
    // 7. Delete the blob (currently a no-op in DirectWalrusBlobStore, but the DB record is removed).
    let resp = app
        .clone()
        .oneshot(
...
async fn e2e_content_dedup() {
    ...
    let store = |bucket: &str, key: &str, data: &[u8]| {
    ...
    let req1 = store(&bucket_id, &api_key, data).await;
    let (s1, b1) = json_response(app, req1).await;
    assert_eq!(s1, axum::http::StatusCode::CREATED);

    let req2 = store(&bucket_id, &api_key, data).await;
...
#[tokio::test]
#[ignore = "requires walrus test cluster (~30s startup)"]
async fn e2e_wallet_provisioning() {
    let _ = rustls::crypto::aws_lc_rs::default_provider().install_default();
    tracing_subscriber::fmt::try_init().ok();
